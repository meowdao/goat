...
import AccessTokenController from "../../oauth2/controllers/access-token";
...
passport.use(new LocalStrategy(config.strategies.local,
	(email, password, callback) => {
		...
		userController.findOne({email: email.toLowerCase()}, {
			...
			.tap(user => {
				if (!user) {
					throw makeError("unauthorized", 401, {reason: "user-incorrect-name-or-password"});
				}
			})
			.tap(user => {
			...
			.tap(user => {
				if (user.status !== UserController.statuses.active) {
					throw makeError("access-denied", 403, {reason: "not-active"});
				}
			})
			.tap(user => {
				if (!user.isEmailVerified) {
					throw makeError("access-denied", 403, {reason: "email-not-verified"});
				}
			})
			.tap(user => {
