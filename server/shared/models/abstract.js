...
export default class AbstractModel {
	...
	aggregate(query) {
		return bluebird.promisify(this.model.aggregate.bind(this.model))(query);
	}

	mapReduce(query) {
		return bluebird.promisify(this.model.mapReduce.bind(this.model))(query);
	}

	fulltextsearch(text, options) {
		return this.find({$text: {$search: text}}, options);
	}
}
